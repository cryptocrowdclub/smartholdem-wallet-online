...
const TRANSACTION_HEADER_OS = 'smartholdem-lite-wallet'
...
app.factory('$peer', ($http, $log, $q, $timeout) => {
  return class $peer {
    constructor ({ host, port = 6100, ssl = false }) {
      this.host = host
      this.port = port
      this.ssl = !!ssl
      this.enabled = false
    ...
    sendTransaction (recipient, amount, smartevent, passphrase, secondPassphrase) {
      let transaction
